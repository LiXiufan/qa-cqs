...
def Hadamard_test(n, U1, U2, alpha='r', backend='eigens', shots=1024, device='SV1'):
    if backend == 'eigens':
        return Hadamard_test_eigens(U, alpha)
    elif backend == 'braket':
        return Hadamard_test_braket(n, U1, U2, alpha=alpha, device=device, shots=shots)
    elif backend == 'qibo':
        return Hadamard_test_qibo(U, alpha, shots)
    elif backend == 'qibo_noisy':
        return Hadamard_test_qibo_noisy(U, alpha, shots)
    elif backend == 'ionq':
        ...
        return Hadamard_test_braket_noisy(U, alpha, shots)
    else:
