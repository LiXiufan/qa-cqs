...
try:
    ...
    print("No job_id provided! Run with: python func_in_all_HPC.py <job_id>")
    sys.exit(1)
...
#new read_csv_b() function written which is compatible for running on the HPC
"""
def read_csv_b(n):
    file_path = Path(f"./{n}_b_random_circuits.csv")
    # Check if the file exists
    if file_path.exists():
        print(f"Reading file: {file_path}")
        data_b = pd.read_csv(file_path)
...
#simplified read_csv_b() function -- works when you know for sure that the b vector file is uploaded to HPC:

def read_csv_b(n):
    return pd.read_csv(f"{n}_b_random_circuits.csv")
...
with open('3_qubit_data_generation_matrix_A_HPC.csv', 'r', newline='') as csvfile:
    data_b = read_csv_b(3)
    reader = csv.reader(csvfile, delimiter=' ', quotechar='|')
    
    for i, row in enumerate(reader):
        if i == job_id + 1:  # Adjusting for the header row
            row_clean = [j for j in ''.join(row).split('"') if j != ',']
            nLc = row_clean[0].split(',')
            n = int(nLc[0])
            L = int(nLc[1])
            kappa = float(nLc[3])  # column 4 is condition number
            ...
            coeffs = [float(x) for x in eval(row_clean[2])]
            print('coefficients are:', coeffs)
            print()

            # Load b circuit
            ub = qasm3.loads(data_b.iloc[i - 1].qasm)  # i-1 to align with 0-based index
            print('Ub is given by:', data_b.iloc[i - 1].b)
            print(ub)
            ...
            output_file = os.path.join(output_dir, f"new_ls_job_{job_id}_nq_{n}.csv")

            with open(output_file, mode='w', newline='') as f:
                writer = csv.writer(f)

                writer.writerow(["job_id", job_id])
                writer.writerow(["n_qubits", n])
                writer.writerow(["term_count", L])
                writer.writerow(["condition_number", kappa])
                writer.writerow([])

                writer.writerow(["Iteration", "Loss"])
                for itr_val, loss_val in zip(Itr, LOSS):
                    writer.writerow([itr_val, loss_val])

                writer.writerow([])
                writer.writerow(["Ansatz Tree"])
                for a in ansatz_tree:
                    writer.writerow([str(a)])  # Save ASCII circuit
