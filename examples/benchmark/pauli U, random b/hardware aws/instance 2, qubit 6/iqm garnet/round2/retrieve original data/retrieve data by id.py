        ...
        for j in range(i, tree_depth):
            ...
            for k in range(num_term):
                for l in range(num_term):
                    ...
                    count_r = retrieve_data(ip_id[0][0])
                    count_i = retrieve_data(ip_id[0][1])
                    measurement_correspondance = ip_id[1]
                    element_counts[k][l] = ((count_r, count_i), measurement_correspondance)
...
def retrieve_original_data_q(num_term, tree_depth, ip_idxes):
    ...
    for i in range(tree_depth):
        ...
        for k in range(num_term):
            ...
            count_i = retrieve_data(ip_id[0][1])
            measurement_correspondance = ip_id[1]
            element_counts[k] = ((count_r, count_i), measurement_correspondance)
...
V_dagger_V_counts = retrieve_original_data_V_dagger_V(6, 4, ip_idxes=V_dagger_V_idxes)
# Create DataFrame
V_dagger_V_df = pd.DataFrame(V_dagger_V_counts)
# Save to CSV
V_dagger_V_counts_csv_filename = "V_dagger_V_counts_second_round.csv"
V_dagger_V_df.to_csv(V_dagger_V_counts_csv_filename, index=False)


q_counts = retrieve_original_data_q(6, 4, ip_idxes=q_idxes)
# Create DataFrame
q_df = pd.DataFrame(q_counts)
# Save to CSV
q_counts_csv_filename = "q_counts_second_round.csv"
