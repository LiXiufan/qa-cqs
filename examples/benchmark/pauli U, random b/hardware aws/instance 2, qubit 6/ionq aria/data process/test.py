...
from torch import Tensor, matmul, transpose
...
def calculate_V_dagger_V_from_counts(instance, tree_depth, counts):
    r"""
        ...
        the dagger of each estimated inner product. Thus the symmetry is maintained.
...
with open('6_qubit_data_generation_matrix_A.csv', 'r', newline='') as csvfile:
    ...
    for i, row in enumerate(reader):
        if i == 2995:
            ...
            Q_pd = pd.read_csv(hardware_result_Q_csv_filename).values
            r_pd = pd.read_csv(hardware_result_r_csv_filename).values
            Q = array(Q_pd)
            r = array(r_pd)
